...
int main(void)
{
	...
	uint32_t vectorInC[ARRAY_SIZE];
	uint32_t vectorInAsm[ARRAY_SIZE];
	uint32_t vectorOutC[ARRAY_SIZE];
	uint32_t vectorOutAsm[ARRAY_SIZE];
	...
	*DWT_CYCCNT = 0;				//resetea el contador de ciclos
	productoEscalar32EnC(vectorInC, vectorInC, ARRAY_SIZE, escalar);	//1030 ciclos
	cyclesC=*DWT_CYCCNT;			//Guarda la cuenta de ciclos

	*DWT_CYCCNT = 0;
	productoEscalar32EnASM(vectorInAsm, vectorInAsm, ARRAY_SIZE, escalar);	//929 ciclos

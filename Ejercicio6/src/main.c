...
int main(void)
{
	...
	int16_t vectorOutC[ARRAY_SIZE];
	int16_t vectorOutAsm[ARRAY_SIZE];
	...
	*DWT_CYCCNT = 0;				//resetea el contador de ciclos
	pack32to16_C(vectorInC, vectorOutC, ARRAY_SIZE);		// 1825 ciclos
	cyclesC=*DWT_CYCCNT;			//Guarda la cuenta de ciclos

	*DWT_CYCCNT = 0;
	pack32to16_ASM(vectorInAsm, vectorOutAsm, ARRAY_SIZE);	// 1826 ciclos
